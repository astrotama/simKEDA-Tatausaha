<?php
function sppgajiarsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["sppgajiarsip_kodeuk"]))
				$_SESSION["sppgajiarsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["sppgajiarsip_kodeuk"];
			//if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["sppgajiarsip_bulan"]))
				$_SESSION["sppgajiarsip_bulan"]=date('m');
		$bulan = $_SESSION["sppgajiarsip_bulan"];
		//if ($bulan=='') $bulan = date('m');
		
		if(!isset($_SESSION["sppgajiarsip_sppok"]))
				$_SESSION["sppgajiarsip_sppok"]='ZZ';
		$sppok = $_SESSION["sppgajiarsip_sppok"];
		//if ($sppok=='') $sppok = 'ZZ';
		
		if(!isset($_SESSION["sppgajiarsip_jenisgaji"]))
				$_SESSION["sppgajiarsip_jenisgaji"]='ZZ';
		$jenisgaji = $_SESSION["sppgajiarsip_jenisgaji"];
	...
	*/
	...
	$output_form = drupal_get_form('sppgajiarsip_main_form');
	if (isSuperuser()) 
		$header = array (
	...
	$query->fields('d', array('dokid', 'sppno', 'spptgl', 'spmno', 'sp2dno', 'bulan', 'kodeuk', 'jumlah', 'potongan', 'netto', 'sppok', 'keperluan', 'jenisgaji', 'sp2dok', 'sp2dsudah'));
...
function sppgajiarsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["sppgajiarsip_kodeuk"] = $kodeuk;
	$_SESSION["sppgajiarsip_bulan"] = $bulan;
	$_SESSION["sppgajiarsip_sppok"] = $sppok;
	$_SESSION["sppgajiarsip_jenisgaji"] = $jenisgaji;
	
	$uri = 'sppgajiarsip/filter/' . $kodeuk . '/' . $bulan  . '/' . $sppok . '/' . $jenisgaji;
	drupal_goto($uri);
	
}


function sppgajiarsip_main_form($form, &$form_state) {
