<?php
function gantiuangspp_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			$kodeuk = $_SESSION["gantiuangspp_arsip_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		//$bulan = $_SESSION["gantiuangspp_arsip_bulan"];
		//if ($bulan=='') $bulan = date('m');
		$bulan = date('m');
		
		//$sppok = $_SESSION["gantiuangspp_arsip_sppok"];
		//if ($sppok=='') $sppok = 'ZZ';
		$sppok = 'ZZ';
	...
	*/
	...
	$output_form = drupal_get_form('gantiuangspp_arsip_main_form');
	if (isSuperuser()) 
		$header = array (
	...
	$query->fields('d', array('dokid', 'jenisdokumen', 'sppno', 'spmok','spmno', 'sp2dno', 'sp2dok', 'spptgl', 'bulan', 'kodeuk', 'jumlah', 'potongan', 'netto', 'sppok', 'keperluan'));
	...
	foreach ($results as $data) {
		...
		if ($data->sppok=='0') $editlink .=  apbd_button_hapus('guspp/delete/' . $data->dokid);
...
;
	...
	$btn = '';
	if (isUserSKPD()) {
		if (apbd_gutu_aktif()) $btn = apbd_button_baru('gubaruspp/newkeg') . "&nbsp;";
...
function gantiuangspp_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["gantiuangspp_arsip_kodeuk"] = $kodeuk;
	$_SESSION["gantiuangspp_arsip_bulan"] = $bulan;
	$_SESSION["gantiuangspp_arsip_sppok"] = $sppok;
	
	$uri = 'guspparsip/filter/' . $kodeuk . '/' . $bulan  . '/' . $sppok;
	drupal_goto($uri);
	
}


function gantiuangspp_arsip_main_form($form, &$form_state) {
