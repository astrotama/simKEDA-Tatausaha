<?php
function gantiuangspp_newspj_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){		
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('gantiuangspp_newspj_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function gantiuangspp_newspj_main_form($form, &$form_state) {
	...
	$form['formdokumen']['tablespj']= array(
		'#prefix' => '<table class="table table-hover"><tr><th width="10px">No.</th><th width="100px">No. SPJ</th><th width="100px">Tanggal</th><th>Keperluan</th><th>Penerima</th><th width="100px">Jumlah</th><th>Sudah</th><th width="50px">Pilih</th></tr>',
		 '#suffix' => '</table>',
	);	
	$i = 0;
...
function gantiuangspp_newspj_main_form_validate($form, &$form_state) {
	//$sppno = $form_state['values']['sppno'];
		
}
	
function gantiuangspp_newspj_main_form_submit($form, &$form_state) {
