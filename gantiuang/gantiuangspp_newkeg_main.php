<?php
function gantiuangspp_newkeg_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 20;
	...
	if ($kodesuk=='ZZ')
		...
		$str_suk = '| ' . '<a href="/guspp/newkeg/ZZ">SEMUA</a>' . ' |';
	...
	foreach ($query as $data) {
		...
		if ($kodesuk==$data->kodesuk)
			...
			$str_suk .= apbd_blank_space() . '<a href="/guspp/newkeg/' . $data->kodesuk . '">' . $namasuk_menu . '</a>' . ' |';
	...
	$query->condition('d.dispensasi', '1', '=');
	...
	foreach ($results as $data) {
		...
		if (isjurnalsudahuk($data->kodekeg)) {
			...
			$editlink = apbd_button_baru_custom('guspp/newspj/' . $data->kodekeg, 'SPP'); 
		} else {
	...
	//$output_form = drupal_get_form('gantiuangspp_newkeg_main_form');
...
function gantiuangspp_newkeg_main_form_submit($form, &$form_state) {
	/*
	...
	$_SESSION["gantiuangspp_newkeg_bulan"] = $bulan;
	$_SESSION["gantiuangspp_newkeg_spjsudah"] = $spjsudah;
	$_SESSION["gantiuangspp_newkeg_jenisgaji"] = $jenisgaji;
...
function gantiuangspp_newkeg_main_form($form, &$form_state) {
	/*
	$kodeuk = apbd_getuseruk();
	if(arg(2)!=null){
		...
		//$bulan = date('m');
		isset($_SESSION["gantiuangspp_newkeg_bulan"])?$bulan = $_SESSION["gantiuangspp_newkeg_bulan"]:$bulan = date('m');;
		isset($_SESSION["gantiuangspp_newkeg_spjsudah"])?$spjsudah = $_SESSION["gantiuangspp_newkeg_bulan"]:$spjsudah = 'ZZ';
		isset($_SESSION["gantiuangspp_newkeg_jenisgaji"])?$jenisgaji = $_SESSION["gantiuangspp_newkeg_jenisgaji"]:$jenisgaji = 'ZZ';
