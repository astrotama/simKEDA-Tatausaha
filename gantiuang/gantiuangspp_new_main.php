<?php
function gantiuangspp_new_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){		
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('gantiuangspp_new_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function gantiuangspp_new_main_form($form, &$form_state) {
...
function gantiuangspp_new_main_form_validate($form, &$form_state) {
...
function gantiuangspp_new_main_form_submit($form, &$form_state) {
	...
	//try {
		...
		$query = db_insert('dokumen')
				->fields(array('dokid', 'bulan', 'kodekeg', 'jenisdokumen', 'kodeuk', 'sppno', 'spptgl', 'spmtgl', 'keperluan', 'jumlah', 'netto', 'penerimanama', 'penerimanip', 'penerimabanknama', 'penerimabankrekening', 'penerimanpwp', 'pptknama', 'pptknip'))
				->values(
					array(
						'dokid'=> $dokid,
						'bulan'=> 1,
