...
function gantiuangspp_edit_main($arg=NULL, $nama=NULL) {
	...
	if($print=='spp1') {			  
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('gantiuangspp_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function gantiuangspp_edit_main_form($form, &$form_state) {
	...
	if($gambar>0){
		...
		$form['formcetak']['submitgambarspj']= array(
			'#type' => 'submit',
			'#value' =>  '<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> S P J',
			'#attributes' => array('class' => array('btn btn-success btn-sm pull-right')),
		);
	...
	*/
	...
	/*
	$form['formcetak']['submitsptjm']= array(
		'#type' => 'submit',
...
function gantiuangspp_edit_main_form_validate($form, &$form_state) {
