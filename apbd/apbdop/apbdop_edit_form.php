	...
	foreach( array_keys($roles) as $rid) {
		...
		switch ($roles[$rid]) {
			...
			case 'verifikator':
				...
				break;
			case 'verifikatorskpd':
				$arr_akses_by_id[$rid] = 'Verifikator SKPD';
				break;
...
function apbdop_edit_form_validate($form, &$form_state) {
	...
	if ($username != $e_username) {
		...
		if ($ada) {
			form_set_error('username', 'username ' . $username . ' sudah dipakai, pilih yang lain.');
		}
		
        if (strlen($form_state['values']['upwd']) < 5 ) {
