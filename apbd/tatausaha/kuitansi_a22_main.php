...
function kuitansi_a22_main($arg=NULL, $nama=NULL) {
	...
	if($print=='a22') {			
		...
		$output = printspp_a22($dokid, $kodero, $tanggal, $penerimanama, $penerimanalamat, $jumlah, $bendaharanama, $bendaharanip, $bendaharajabatan, $ppknama, $ppknip);
...
function kuitansi_a22_main_form($form, &$form_state) {
	...
	$query = db_select('dokumen', 'b');
	$query->join('dokumenrekening', 'bi', 'b.dokid=bi.dokid');
	$query->join('rincianobyek', 'ro', 'bi.kodero=ro.kodero');
	$query->join('unitkerja', 'uk', 'b.kodeuk=uk.kodeuk');
	$query->join('kegiatanskpd', 'k', 'bi.kodekeg=k.kodekeg');
	
	$query->fields('b', array('spptgl', 'keperluan', 'penerimanama'));
	...
	foreach ($results as $data) {
		...
		$tanggal = apbd_fd_long($data->spptgl);
...
function kuitansi_a22_main_form_submit($form, &$form_state) {
	...
	drupal_goto('kuitansi/edita2/' . $dokid . '/' . $kodero . '/a22/' . $tanggal . '/'. $penerimanama . '/'. $penerimanalamat . '/' . 
		$jumlah . '/' . $bendaharanama . '/' . $bendaharanip . '/' . $bendaharajabatan . '/' . $ppknama . '/' . $ppknip );
	
}

function printspp_a22($dokid, $kodero, $tanggal, $nama, $alamat, $jumlah, $bendaharanama, $bendaharanip, $bendaharajabatan, $ppknama, $ppknip) {
