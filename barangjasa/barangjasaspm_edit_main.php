<?php
function barangjasaspm_edit_main($arg=NULL, $nama=NULL) {
	...
	if (arg(3)=='pdf-e') {			 
		...
		$kodeuk = '00';
		$output = printspm_ls($dokid, $kodeuk);
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('barangjasaspm_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function barangjasaspm_edit_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'arsip')>0)
		$_SESSION["barangjasaspmlastpage"] = $referer;
	else
	...
	$spdno = '';
	$spdtgl = '';
	$keperluan = 'SPM Barang Jasa ' . apbd_tahun();
	$jeniskegiatan = '1';
	$penerimanama = '';
	$penerimapimpinan = '';
	$penerimaalamat = '';
	...
	$query->fields('d', array('dokid', 'sppno', 'spmno', 'spmtgl', 'spptgl', 'kodekeg', 'bulan', 'keperluan', 'jumlah', 'pajak', 'penerimanama', 'penerimabankrekening', 'penerimabanknama', 'penerimanpwp', 'pptknama', 'pptknip', 'sp2dok', 'spmok', 'cetakspm'));
...
function barangjasaspm_edit_main_form_validate($form, &$form_state) {
...
function barangjasaspm_edit_main_form_submit($form, &$form_state) {
$dokid = $form_state['values']['dokid'];

if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
	drupal_goto('barangjasaspm/edit/' . $dokid . '/pdf');

} else if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint-e']) {
	drupal_goto('barangjasaspm/edit/' . $dokid . '/pdf-e');
	...
	drupal_goto('barangjasaspm/delete/' . $dokid);
	...
	try {	
		...
		*/
		...
		atchdog_exception('barangjasaspm-' . $nourut, $e);
	}
	
	if($form_state['clicked_button']['#value'] == $form_state['values']['submitspm']) drupal_goto('barangjasaspm/verify/' . $dokid);

	//drupal_goto('barangjasaspmarsip');
	//drupal_goto();
}	 
}

function printspm_ls($dokid, &$kodeuk) {
...
function printspm_digital($dokid, &$kodeuk) {
