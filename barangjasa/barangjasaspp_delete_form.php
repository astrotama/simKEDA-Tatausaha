...
function barangjasaspp_delete_form() {
    ...
    if (isset($dokid)) {
        ...
        if ($ada) {
			...
			if ($current_url != $referer)
				$_SESSION["barangjasaspp_lastpage"] = $referer;
			else
				$referer = $_SESSION["barangjasaspp_lastpage"];
...
function barangjasaspp_delete_form_validate($form, &$form_state) {
}
function barangjasaspp_delete_form_submit($form, &$form_state) {
