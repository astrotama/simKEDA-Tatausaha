<?php
function barangjasasp2d_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["barangjasasp2d_arsip_kodeuk"]))
				$_SESSION["barangjasasp2d_arsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["barangjasasp2d_arsip_kodeuk"];
			//if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["barangjasasp2d_arsip_bulan"]))
				$_SESSION["barangjasasp2d_arsip_bulan"]='1';
		$bulan = $_SESSION["barangjasasp2d_arsip_bulan"];
		//if ($bulan=='') $bulan = '1';
		
		if(!isset($_SESSION["barangjasasp2d_arsip_sp2dok"]))
				$_SESSION["barangjasasp2d_arsip_sp2dok"]='ZZ';
		$sp2dok = $_SESSION["barangjasasp2d_arsip_sp2dok"];
		//if ($sp2dok=='') $sp2dok = 'ZZ';
	...
	$output_form = drupal_get_form('barangjasasp2d_arsip_main_form');
	if (isSuperuser())
		$header = array (
	...
	foreach ($results as $data) {
		...
		if($data->sp2dok=='2')
			...
			//CETAK
			//if ((isSuperuser()) and  ($data->sp2dno!='')) $editlink .= apbd_button_cetak('barangjasasp2d/edit/' . $data->dokid . '/pdf');
		...
		if (isSuperuser()) {
			...
			if (($data->sp2dok=='1') and  ($data->sp2dno!='')) 
				$cetaklink = apbd_button_cetak('barangjasasp2d/edit/' . $data->dokid . '/pdf');
			else
				$cetaklink = 'Cetak';
...
function barangjasasp2d_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["barangjasasp2d_arsip_kodeuk"] = $kodeuk;
	$_SESSION["barangjasasp2d_arsip_bulan"] = $bulan;
	$_SESSION["barangjasasp2d_arsip_sp2dok"] = $sp2dok;

	$uri = 'barangjasasp2darsip/filter/' . $kodeuk . '/' . $bulan . '/' . $sp2dok ;
	drupal_goto($uri);
	
}


function barangjasasp2d_arsip_main_form($form, &$form_state) {
