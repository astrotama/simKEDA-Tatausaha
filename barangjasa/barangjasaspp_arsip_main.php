<?php
function barangjasaspp_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["barangjasaspp_arsip_kodeuk"]))
				$_SESSION["barangjasaspp_arsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["barangjasaspp_arsip_kodeuk"];
			//if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["barangjasaspp_arsip_bulan"]))
			$_SESSION["barangjasaspp_arsip_bulan"] = date('m');
		$bulan = $_SESSION["barangjasaspp_arsip_bulan"];
		//if ($bulan=='') $bulan = date('m');
		if(!isset($_SESSION["barangjasaspp_arsip_sppok"]))
				$_SESSION["barangjasaspp_arsip_sppok"]='ZZ';
		$sppok = $_SESSION["barangjasaspp_arsip_sppok"];
		//if ($sppok=='') $sppok = 'ZZ';
	...
	*/
	...
	$output_form = drupal_get_form('barangjasaspp_arsip_main_form');
	if (isSuperuser()) 
		$header = array (
	...
	$query->fields('d', array('dokid', 'jenisdokumen', 'sppno', 'spmno', 'sp2dno', 'sp2dok', 'spptgl', 'bulan', 'kodeuk', 'jumlah', 'potongan', 'netto', 'sppok', 'keperluan', 'penerimanama'));
	...
	foreach ($results as $data) {
		...
		if($data->sppok=='0') $editlink .=  apbd_button_hapus('barangjasaspp/delete/' . $data->dokid);
	...
	/*
	if (isUserSKPD()) {
		$btn = apbd_button_baru('barangjasaspp/newkeg');
...
function barangjasaspp_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["barangjasaspp_arsip_kodeuk"] = $kodeuk;
	$_SESSION["barangjasaspp_arsip_bulan"] = $bulan;
	$_SESSION["barangjasaspp_arsip_sppok"] = $sppok;
	
	$uri = 'barangjasaspparsip/filter/' . $kodeuk . '/' . $bulan  . '/' . $sppok;
	drupal_goto($uri);
	
}


function barangjasaspp_arsip_main_form($form, &$form_state) {
