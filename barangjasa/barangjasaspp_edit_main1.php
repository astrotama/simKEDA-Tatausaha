...
function barangjasaspp_edit_main($arg=NULL, $nama=NULL) {
	...
	if($print=='spp1') {			  
		...
		$edoc = 'http://simkedajepara.web.id/edoc2019/spm/E_SPJM_' . $dokid . '.PDF';
		if (is_eDocExists($edoc)) {
			drupal_goto($edoc); 
...
function barangjasaspp_edit_main_form($form, &$form_state) {
	...
	//CETAK ATAS	
	/*
	$form['formcetak']['submitkontrak']= array(
		'#type' => 'item',
		'#markup' => '&nbsp' . l('<span class="glyphicon glyphicon-print" aria-hidden="true"></span> Kontrak', 'kontrak/edit/' . $dokid , array ('html' => true, 'attributes'=> array ('class'=>'btn btn-success btn-sm pull-right'))),
	);
	*/
	...
	*/
	...
	$edocsptjm = 'http://simkedajepara.web.id/edoc2019/spm/E_SPJM_' . $dokid . '.PDF';
	if (is_eDocExists($edocsptjm)) {
		$form['formdata']['submitsptjm']= array(
			'#type' => 'submit',
			'#value' => '<span class="glyphicon glyphicon-print" aria-hidden="true"></span> SPTJM',
			'#attributes' => array('class' => array('btn btn-succes btn-sm pull-right')),
		);		
	} else {	
		$form['formdata']['submitsptjm']= array(
			'#type' => 'submit',
	...
	$edocspkt = 'http://simkedajepara.web.id/edoc2019/spm/E_SPKT_' . $dokid . '.PDF';
	if (is_eDocExists($edocspkt)) {
	...
	$form['formdata']['submitsppkon']= array(
		'#type' => 'submit',
		'#value' => '<span class="glyphicon glyphicon-print" aria-hidden="true"></span> Kontrak',
		'#attributes' => array('class' => array('btn btn-success btn-sm pull-right')),
	);
...
function barangjasaspp_edit_main_form_validate($form, &$form_state) {
	if($form_state['clicked_button']['#value'] == $form_state['values']['submitspm']) {
		...
		if ($jumlahrekening > 15) {
			form_set_error('jumlahapbd1', 'Banyaknya rekening yang diajukan dalam satu SPP maksimal 15 buah');
		}				
