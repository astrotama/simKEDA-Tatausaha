<?php
function barangjasasp2d_edit_main($arg=NULL, $nama=NULL) {
	...
	if (arg(3)=='pdf0') {			
		...
		//$output .= theme('pager');
		//drupal_set_message();
		$output_form = drupal_get_form('barangjasasp2d_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function barangjasasp2d_edit_main_form($form, &$form_state) {

	//FORM NAVIGATION	
	$current_url = url(current_path(), array('absolute' => TRUE));
	$referer = $_SERVER['HTTP_REFERER'];
	if ($current_url != $referer)
		$_SESSION["barangjasasp2dlastpage"] = $referer;
	else
		$referer = $_SESSION["barangjasasp2dlastpage"];
	...
	$query->fields('d', array('dokid', 'spmno', 'sp2dno', 'sp2dtgl', 'kodekeg', 'bulan', 'keperluan', 'jumlah', 'pajak', 
	...
	*/
	...
	$form['formrekening']['table']= array(
		'#prefix' => '<table class="table table-hover"><tr><th width="10px">NO</th><th width="90px">KODE</th><th>URAIAN</th><th width="90px">ANGGARAN</th><th width="50px">CAIR</th><th width="90px">BATAS</th><th width="90px">JUMLAH</th><th width="90px">SISA</th></tr>',
		 '#suffix' => '</table>',
	);	
	...
	foreach ($results as $data) {
		...
		drupal_set_message($dokid . '|' . $kodekeg . '|' . $data->kodero . '|' . $tanggalsql);
	...
	foreach ($results as $data) {
		...
		if ($sudahbayar=='0')
			...
			$uraian = '<p style="color:blue">' . $data->uraian . ' (sudah dibayar lunas)</p>';
	...
	if ($i==0) {
		$i++; 
		$uraian = '<p style="color:red">Tidak ada pajak</p>';
...
function barangjasasp2d_edit_main_form_validate($form, &$form_state) {
...
if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
...
}else if($form_state['clicked_button']['#value'] == $form_state['values']['submitsp2dkoreksi']) {
	...
	$ret = db_query('INSERT INTO dokumenrekeningtemp (dokid, kodero, jumlah, kodekeg, sumber, spjid) SELECT dokid, kodero, jumlah, kodekeg, sumber, spjid FROM dokumenrekening  WHERE dokid =:dokid',array(':dokid'=>$dokid));
	$ret = db_query('update dokumenrekeningtemp set dokid = :newdokid',array(':newdokid'=>$newdokid));
	$ret = db_query('INSERT INTO dokumenrekening (dokid, kodero, jumlah, kodekeg, sumber, spjid) SELECT dokid, kodero, jumlah, kodekeg, sumber, spjid FROM dokumenrekeningtemp');
	$ret = db_query('DELETE FROM `dokumenrekeningtemp` WHERE 1');
	...
	if ($penerimabankrekening=='1015000917') $penerimabankrekening=='10150000917';
	...
	try {	
		...
		atchdog_exception('barangjasasp2d-' . $nourut, $e);
	}
...
function printsp2d($dokid) {
	...
	$rows[]=array(
		...
		array('data' => '', 'width' => '45px','align'=>'left','style'=>'font-size:150%;border:none;'),
	...
	foreach ($results as $data) {
		...
		$uraian = $data->uraian;
		if ($data->sudahbayar) $uraian .= ' (Pajak sudah dibayar)';
		$rows[]=array(
			array('data' => $n, 'width' => '30px','align'=>'center','style'=>'border-right:1px solid black;'),
			array('data' => $uraian, 'width' => '270px','align'=>'left','style'=>'border-right:1px solid black;'),
			array('data' => apbd_fn($data->jumlah), 'width' => '90px','align'=>'right','style'=>'border-right:1px solid black;'),
			array('data' => $data->keterangan, 'width' => '90px','align'=>'left','style'=>''),
		);
	}
...
function sp2dgbr($data,$tot){
	...
	for($n=0;$n<$tot;$n++){
		...
		$string.='<td   valign="top" align="left" width="150px"><a data-fancybox="gallery" href="http://tu18.simkedajepara.com'.$data[0][$n].'"><img  class="rotateimg" src="http://tu18.simkedajepara.com'.$data[0][$n].'" width="150px" heigh="150px" ></img></a></td>';
		if(($n+1)==$tot){
			$string.="</tr>";
		}
		//Hapus...............
