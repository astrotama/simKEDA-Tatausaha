<?php
function sp2duptuarsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["sp2duptuarsip_kodeuk"]))
				$_SESSION["sp2duptuarsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["sp2duptuarsip_kodeuk"];
			//if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["sp2duptuarsip_bulan"]))
				$_SESSION["sp2duptuarsip_bulan"]='1';
		$bulan = $_SESSION["sp2duptuarsip_bulan"];
		//if ($bulan=='') $bulan = '1';

		if(!isset($_SESSION["sp2duptuarsip_jenisdokumen"]))
				$_SESSION["sp2duptuarsip_jenisdokumen"]='ZZ';
		$jenisdokumen = $_SESSION["sp2duptuarsip_jenisdokumen"];
		//if ($jenisdokumen=='') $jenisdokumen = 'ZZ';
		
		if(!isset($_SESSION["sp2duptuarsip_sp2dok"]))
				$_SESSION["sp2duptuarsip_sp2dok"]='ZZ';
		$sp2dok = $_SESSION["sp2duptuarsip_sp2dok"];
		//if ($sp2dok=='') $sp2dok = 'ZZ';
	}
	...
	$output_form = drupal_get_form('sp2duptuarsip_main_form');
	if (isSuperuser()) {
		$header = array (
	...
	$query->fields('d', array('dokid', 'bulan', 'keperluan', 'kodeuk', 'spmno', 'sppno', 'sp2dno', 'sp2dtgl', 'jumlah', 'sp2dok'));
	...
	*/
	...
	foreach ($results as $data) {
		...
		if($data->sp2dok=='2')
			...
			//CETAK
			//f ((isSuperuser()) and  ($data->sp2dno!='')) $editlink .= apbd_button_cetak('sp2duptu/edit/' . $data->dokid . '/pdf');
		...
		if (isSuperuser()) {
			...
			if (($data->sp2dok=='1') and  ($data->sp2dno!='')) 
				$cetaklink = apbd_button_cetak('sp2duptu/edit/' . $data->dokid . '/pdf');
			else
				$cetaklink = 'Cetak';
...
function sp2duptuarsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["sp2duptuarsip_kodeuk"] = $kodeuk;
	$_SESSION["sp2duptuarsip_bulan"] = $bulan;
	$_SESSION["sp2duptuarsip_jenisdokumen"] = $jenisdokumen;
	$_SESSION["sp2duptuarsip_sp2dok"] = $sp2dok;

	$uri = 'sp2duptuarsip/filter/' . $kodeuk . '/' .  $bulan . '/' . $jenisdokumen . '/' . $sp2dok ;
	drupal_goto($uri);
	
}


function sp2duptuarsip_main_form($form, &$form_state) {
	...
	*/
	...
	$form['formdata'] = array (
		'#type' => 'fieldset',
		'#title'=>  'PILIHAN DATA' . '<em><small class="text-info pull-right">' . get_label_data($bulan, $sp2dok) . '</small></em>',
	...
	$opt_jenisdokumen['ZZ'] ='UP DAN TU';
	$opt_jenisdokumen['0'] = 'UP SAJA';
	$opt_jenisdokumen['2'] = 'TU SAJA';	
