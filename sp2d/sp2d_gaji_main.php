<?php
function sp2d_gaji_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			$kodeuk = $_SESSION["sp2d_gaji_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		$bulan = $_SESSION["sp2d_gaji_bulan"];
		if ($bulan=='') $bulan = '1';
	...
	$output_form = drupal_get_form('sp2d_gaji_main_form');
	if (isSuperuser())
		$header = array (
	...
	$query->fields('d', array('dokid', 'bulan', 'keperluan', 'kodeuk', 'spmno', 'sppno', 'sp2dno', 'sp2dtgl', 'jumlah', 'potongan', 'netto', 'sp2dok', 'jenisgaji', 'cetaksp2d'));
...
function sp2d_gaji_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["sp2d_gaji_kodeuk"] = $kodeuk;
	$_SESSION["sp2d_gaji_bulan"] = $bulan;
	$_SESSION["sp2d_gaji_sp2dok"] = $sp2dok;
	$_SESSION["sp2d_gaji_jenisgaji"] = $jenisgaji;

	$uri = 'sp2darsipgaji/filter/' . $kodeuk . '/' . $bulan . '/' . $sp2dok . '/' . $jenisgaji;
	drupal_goto($uri);
	
}


function sp2d_gaji_main_form($form, &$form_state) {
