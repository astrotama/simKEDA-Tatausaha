 ...
 */
function verifikasispmgu_spp2_form($form, &$form_state) {
	$dokid = arg(2);
	if ($dokid=='') {
...
function verifikasispmgu_spp2_form_submit($form, &$form_state) {
	goback();
}

function verifikasispmgu_bk5_form($form, &$form_state) {
	...
	foreach ($reskeg as $datakeg) {
		...
		$bk6  = '<a href="/verifikasispmgu/bk6keg/' . $dokid . '/' . $datakeg->kodekeg . '">BK-6</a>';
		$form['formkegiatan']['kegiatan'. $nokeg]= array(
			'#type'         => 'checkbox', 
			'#title' =>  t($nokeg . '. ' . $datakeg->kegiatan),
			'#default_value'=> 0 , 
...
function verifikasispmgu_bk5_form_submit($form, &$form_state) {
	goback();
}

function verifikasispmgu_bk8_form() {
...
function verifikasispmgu_bk8_form_submit($form, &$form_state) {
	goback();
}

function verifikasispmgu_bk6_form($form, &$form_state) {
	$dokid = arg(2);
	if ($dokid=='') {
		$dokid = $_SESSION["verifikasi_spm_gu"];
	} else {
...
function verifikasispmgu_bk6_form_submit($form, &$form_state) {
	goback();
}

function verifikasispmgu_verify_form() {
	$dokid = arg(2);
	if ($dokid=='') {
		$dokid = $_SESSION["verifikasi_spm_gu"];
	} else {
	...
	$query = db_select('dokumen', 'd');
	$query->fields('d', array('dokid', 'spmok'));
	$query->condition('d.dokid', $dokid, '=');
	$results = $query->execute();
	foreach ($results as $data) {
		$spmok = $data->spmok;
...
function verifikasispmgu_verify_form_validate($form, &$form_state) {
...
function verifikasispmgu_verify_form_submit($form, &$form_state) {
	$dokid = $form_state['values']['dokid'];
	$jumlahrekkelengkapan = $form_state['values']['jumlahrekkelengkapan'];
	$jumlahrekkelengkapanspm = $form_state['values']['jumlahrekkelengkapanspm'];
...
function goback() {
	$dokid = $_SESSION["verifikasi_spm_gu"];
	drupal_goto('gubaruspm/edit/' . $dokid);
}
...
function verifikasi_bk5($dokid, $kodeuk, $kodekeg, $tglawal, $tglakhir){
	...
	foreach ($res_rek as $data_rek) {
		...
		$uraian = '<a href="/verifikasispmgu/bk6rek/' . $dokid . '/' . $kodekeg . '/' . $data_rek->kodero . '">' .  $data_rek->uraian . '</a>';
