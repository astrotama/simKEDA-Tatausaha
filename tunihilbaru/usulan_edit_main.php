...
function usulan_edit_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'arsip')>0)
		$_SESSION["usulantunihillastpage"] = $referer;
	else
		$referer = $_SESSION["usulantunihillastpage"];
	
	$spjid = arg(2);


	$result = db_query('SELECT s.spjid, s.kodeuk, s.kodekeg, s.tanggal, s.keperluan, s.jumlah, s.posting, d.spptgl, d.sppno, s.tglakhir, s.tglawal, s.spdno, s.bk5url, s.bk6url, s.spp2url, s.spjlink FROM spjtu s left join dokumen d ON s.dokid=d.dokid WHERE s.spjid =:spjid', array(':spjid' => $spjid));
	...
	$form['formcetak']['submitgambar']= array(
		'#type' => 'item',
		'#markup' => apbd_button_image('usulan_upload/edit/' . $spjid . '/tu', $adagambar),
	);
	...
	*/
	...
	foreach ($results as $data) {
		...
		$linkprint1 = apbd_link_print_small('/tukuitansi/edit/' . $spjid . '/' . $kodero , '');
		$linkprint2 = apbd_link_print_small('/tukuitansi/edita2/' . $spjid . '/' . $kodero , '');
...
function usulan_edit_main_form_submit($form, &$form_state) {
	...
	if ($form_state['clicked_button']['#value'] == $form_state['values']['submitA21']) {
		drupal_goto('/tunihilkuitansi/edit/' . $spjid . '/ZZ');

	} else if ($form_state['clicked_button']['#value'] == $form_state['values']['submitspp3']) {
		drupal_goto('/tunihilbaru/edit/' . $spjid . '/spp3');
...
function printspp_2($spjid) {
	...
	$query = db_select('spd', 's');
	$query->fields('s', array('spdkode', 'spdno', 'spdtgl', 'jumlah', 'tw'));
	if ($kodeuk != '00')  $query->condition('s.jenis',7, '=');	
	$query->condition('s.kodeuk', $kodeuk, '=');	
	# execute the query	
	$results = $query->execute();
	...
	$rows[]=array(
		array('data' => 'PEMBAYARAN TU NIHIL [SPP-TU]', 'width' => '510px','align'=>'center','style'=>'border:1px solid black;'),
	);
...
function printspp_3($spjid){
	...
	$rows[]=array(
		array('data' => 'USULAN PEMBAYARAN TU NIHIL [SPP-TU]', 'width' => '510px','align'=>'center','style'=>'border:1px solid black;'),
	);
	$rows[]=array(
