<?php
function tunihilkuitansi_edit_main($arg=NULL, $nama=NULL) {
	...
	if($print=='a21') {			
		...
		$output_form = drupal_get_form('tunihilkuitansi_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
		
}

function tunihilkuitansi_edit_main_form($form, &$form_state) {
	...
	if ($kodero != 'ZZ') {
		...
		$query->join('spjturekening', 'dr', 'ro.kodero=dr.kodero');
...
function tunihilkuitansi_edit_main_form_submit($form, &$form_state) {
	...
	drupal_goto('tunihilkuitansi/edit/' . $spjid . '/' . $kodero . '/a21/' . $tanggal . '/'. $penerimanama . '/'. $penerimanalamat . '/' . $jumlah);
