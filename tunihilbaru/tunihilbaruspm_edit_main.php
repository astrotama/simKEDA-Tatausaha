<?php
function tunihilbaruspm_edit_main($arg=NULL, $nama=NULL) {
	...
	if (arg(3)=='pdf-e') {			
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('tunihilbaruspm_edit_main_form');
...
function tunihilbaruspm_edit_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'arsip')>0)
		$_SESSION["spmtunihillastpage"] = $referer;
	else
		$referer = $_SESSION["spmtunihillastpage"];
...
function tunihilbaruspm_edit_main_form_validate($form, &$form_state) {
...
if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
	...
	drupal_goto('tunihilspm/delete/' . $dokid);
...
function printspm($dokid, &$kodeuk) {
	...
	$rows[]=array(
		array('data' => 'SURAT PERINTAH MEMBAYAR (SPM) - TU NIHIL', 'width' => '510px','align'=>'center','style'=>'border:none;font-size:150%;font-weight:bold;text-decoration: underline;'),
	);
	$rows[]=array(
