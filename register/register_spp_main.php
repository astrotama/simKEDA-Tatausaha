<?php
function register_spp_main($arg=NULL, $nama=NULL) {
	$kodeuk = arg(1);
	$bulan = arg(2);
	$sumberdana = arg(3);
	$jenisdokumen=arg(4);
	...
	if (arg(1)=='kegiatan') {
		...
		apbd_ExportPDF('L', 'F4', $output, 'Register SPP.pdf');
		...
		//$output_form = drupal_get_form('register_spp_main_form');
...
function register_spp_main_form($form, &$form_state) {
...
function register_spp_main_form_validate($form, &$form_state) {
...
function register_spp_main_form_submit($form, &$form_state) {
	$kodeuk = $form_state['values']['kodeuk'];
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['cetakpdf']){
		drupal_goto('cetakregisterspp/' . $kodeuk . '/' . $bulan . '/' . $sumberdana . '/' . $jenisdokumen . '/' . $margin . '/'. $tanggal . '/pdf');
	} else if($form_state['clicked_button']['#value'] == $form_state['values']['cetak']){
		drupal_goto('cetakregisterspp/' . $kodeuk . '/' . $bulan . '/' . $sumberdana . '/' . $jenisdokumen . '/' . $margin . '/'. $tanggal . '/cetak');
	} else if($form_state['clicked_button']['#value'] == $form_state['values']['cetakexcel']){
		drupal_goto('cetakregisterspp/' . $kodeuk . '/' . $bulan . '/' . $sumberdana . '/' . $jenisdokumen . '/' . $margin . '/'. $tanggal . '/excel');
	}
...
function getlaporanregister($kodeuk, $sumberdana, $bulan, $tanggal,$jenisdokumen){
	...
	$rows[]=array(
		array('data' => 'REGISTER SPP' . $labeljenis, 'width' => '875px','align'=>'center','style'=>'font-weight:bold;font-size:80%;'),
	);
	if (!isSuperuser()) {
		$rows[]=array(
	...
	$header=array(
		...
		array('data' => 'NO. SPP',  'width' => '45px','align'=>'center','style'=>'border-top:1px solid black;border-bottom:1px solid black;border-right:1px solid black;font-size:70%;'),
	...
	if ($bulan!='0') {
		$query->where('EXTRACT(MONTH FROM d.spptgl) = :month', array('month' => $bulan));
	}		
	if ($sumberdana != 'ZZ') {
	...
	foreach($results as $data){
		...
		$rows[]=array(
			...
			array('data' => apbd_fd($data->spptgl), 'width' => '45px','align'=>'center','style'=>'border-right:1px solid black;font-size:70%;'),
			array('data' => $data->sppno, 'width' => '45px','align'=>'center','style'=>'border-right:1px solid black;font-size:70%;'),
...
function getlaporanregisterkegiatan($kodekeg){
	...
	//Content
	$results=db_query('SELECT d.dokid, d.spptgl, d.sppno, d.jenisdokumen, d.bulan, d.kodeuk, d.jumlah, d.potongan, d.netto, d.sppok, d.keperluan, d.jenisgaji, d.sppok, d.sp2dsudah, u.namasingkat, d.penerimanama, d.penerimabanknama, d.penerimabankrekening  FROM  {dokumen} d INNER JOIN {unitkerja} u ON d.kodeuk=u.kodeuk WHERE (d.sppok=1) and (d.kodekeg=:kodekeg) ORDER BY d.spptgl ASC,d.sppno ASC ', array(':kodekeg'=>$kodekeg));
...
function getlaporanregister_spp_backup($kodeuk, $bulan, $tanggal,$jenisdokumen){
	set_time_limit(0);
	ini_set('memory_limit','920M');
	...
	if ($bulan=='0') {
		if($kodeuk == 'ZZ') {
			$results=db_query('SELECT d.dokid, d.spptgl, d.sppno, d.jenisdokumen, d.bulan, d.kodeuk, d.jumlah, d.potongan, d.netto, d.sppok, d.keperluan, d.jenisgaji, d.sppok, d.sp2dsudah, u.namasingkat  FROM  {dokumen} d INNER JOIN {unitkerja} u ON d.kodeuk=u.kodeuk WHERE (d.jenisdokumen<>6 and d.jenisdokumen<>8) and d.sppok=1'.$where.'  ORDER BY d.spptgl ASC,d.sppno ASC ');
		
		} else {
			$results=db_query('SELECT d.dokid, d.spptgl, d.sppno, d.jenisdokumen, d.bulan, d.kodeuk, d.jumlah, d.potongan, d.netto, d.sppok, d.keperluan, d.jenisgaji, d.sppok, d.sp2dsudah, u.namasingkat  FROM  {dokumen} d INNER JOIN {unitkerja} u ON d.kodeuk=u.kodeuk WHERE (d.jenisdokumen<>6 and d.jenisdokumen<>8)'.$where.' and (d.sppok=1) and (d.kodeuk=:kodeuk)  ORDER BY d.spptgl ASC,d.sppno ASC ');
		}
		
	} else {
		if($kodeuk == 'ZZ') {
			$results=db_query('SELECT d.dokid, d.spptgl, d.sppno, d.jenisdokumen, d.bulan, d.kodeuk, d.jumlah, d.potongan, d.netto, d.sppok, d.keperluan, d.jenisgaji, d.sppok, d.sp2dsudah, u.namasingkat  FROM  {dokumen} d INNER JOIN {unitkerja} u ON d.kodeuk=u.kodeuk WHERE  (MONTH(d.spptgl) = :bulan)'.$where.' and (d.jenisdokumen<>6 and d.jenisdokumen<>8) and d.sppok=1  ORDER BY d.spptgl ASC,d.sppno ASC ',array(':bulan'=>$bulan));
		
		} else {
			$results=db_query('SELECT d.dokid, d.spptgl, d.sppno, d.jenisdokumen, d.bulan, d.kodeuk, d.jumlah, d.potongan, d.netto, d.sppok, d.keperluan, d.jenisgaji, d.sppok, d.sp2dsudah, u.namasingkat  FROM  {dokumen} d INNER JOIN {unitkerja} u ON d.kodeuk=u.kodeuk WHERE  (MONTH(d.spptgl) = :bulan)'.$where.' and (d.jenisdokumen<>6 and d.jenisdokumen<>8) and (d.sppok=1) and (d.kodeuk=:kodeuk)  ORDER BY d.spptgl ASC,d.sppno ASC ', array(':bulan'=>$bulan, ':kodeuk'=>$kodeuk));
		}
	}
	$n=0;
	...
	foreach($results as $data){
		...
		$rows[]=array(
			...
			array('data' => apbd_fd($data->spptgl), 'width' => '60px','align'=>'center','style'=>'border-right:1px solid black;font-size:80%;'),
			array('data' => $data->sppno, 'width' => '60px','align'=>'center','style'=>'border-right:1px solid black;font-size:80%;'),
	...
	*/
	...
	//SEBELUMNYA
	if($kodeuk == 'ZZ') {
		$results=db_query('SELECT SUM(jumlah) AS total FROM  {dokumen} WHERE  (MONTH(spptgl) < :bulan) and (jenisdokumen<>5 and jenisdokumen<>6 and jenisdokumen<>7 and jenisdokumen<>8) and (sppok=1)',array(':bulan'=>$bulan));
	
	} else {
		$results=db_query('SELECT SUM(jumlah) AS total FROM  {dokumen} WHERE  (MONTH(spptgl) < :bulan) and (jenisdokumen<>5 and jenisdokumen<>6 and jenisdokumen<>7 and jenisdokumen<>8) and (sppok=1) and (kodeuk=:kodeuk)', array(':bulan'=>$bulan, ':kodeuk'=>$kodeuk));
	}
