<?php
function spmnihilantrian_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	$output_form = drupal_get_form('spmnihilantrian_main_form');
	$header = array (
	...
	foreach ($results as $data) {
		...
		if($data->proses=='1'){
			...
			$editlink = apbd_button_jurnalkan('spmnihil/new/' . $data->nourut);
		}
...
function spmnihilantrian_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$uri = 'spmnihilantrian/filter/' . $kodeuk . '/' . $bulan . '/' . $proses ;
	drupal_goto($uri);
	
}


function spmnihilantrian_main_form($form, &$form_state) {
