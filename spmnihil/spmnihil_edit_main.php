<?php
function spmnihil_edit_main($arg=NULL, $nama=NULL) {
	...
	if (arg(3)=='pdf-e') {			
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('spmnihil_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function spmnihil_edit_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'arsip')>0)
		$_SESSION["spmnihillastpage"] = $referer;
	else
		$referer = $_SESSION["spmnihillastpage"];
	...
	foreach ($results as $data) {
		...
		$title = 'SPM Nihil ' . $data->keperluan;
...
function spmnihil_edit_main_form_validate($form, &$form_state) {
...
if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
	...
	drupal_goto('spmnihil/delete/' . $dokid);
	...
	try {	
		...
		atchdog_exception('spmnihil-' . $nourut, $e);
	}
	
	if($form_state['clicked_button']['#value'] == $form_state['values']['submitspmok']) 
		drupal_goto('spmnihil/verify/' . $dokid);

	//drupal_goto('spmnihilarsip');
	//drupal_goto();
}	
}
...
function printspm($dokid, &$kodeuk) {
	...
	$query->fields('d', array('dokid', 'spmno', 'spmtgl', 'sppno', 'spptgl', 'kodekeg', 'bulan', 'keperluan', 'jumlah', 'potongan', 'netto', 
			'penerimanama', 'penerimabankrekening', 'penerimabanknama', 'penerimanpwp', 'spdno', 'spmok', 'jenisdokumen'));
	...
	$rows[]=array(
		array('data' => 'SURAT PERINTAH MEMBAYAR (SPM)  ' . $str_nihil . ' NIHIL', 'width' => '510px','align'=>'center','style'=>'border:none;font-size:150%;font-weight:bold;text-decoration: underline;'),
	);
	$rows[]=array(
