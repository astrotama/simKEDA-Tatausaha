<?php
function tunihilsp2d_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["tunihilsp2d_arsip_kodeuk"]))
				$_SESSION["tunihilsp2d_arsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["tunihilsp2d_arsip_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["tunihilsp2d_arsip_bulan"]))
				$_SESSION["tunihilsp2d_arsip_bulan"]='1';
		$bulan = $_SESSION["tunihilsp2d_arsip_bulan"];
		if ($bulan=='') $bulan = '1';
		
		if(!isset($_SESSION["tunihilsp2d_arsip_sp2dok"]))
				$_SESSION["tunihilsp2d_arsip_sp2dok"]='ZZ';
		$sp2dok = $_SESSION["tunihilsp2d_arsip_sp2dok"];
		if ($sp2dok=='') $sp2dok = 'ZZ';
	...
	$output_form = drupal_get_form('tunihilsp2d_arsip_main_form');
	if (isSuperuser())
		$header = array (
	...
	foreach ($results as $data) {
		...
		$editlink = apbd_button_jurnal('gusp2d/edit/' . $data->dokid);
		...
		if($data->sp2dok=='2')
			...
			//CETAK
			if ((isSuperuser()) and  ($data->sp2dno!='')) $editlink .= apbd_button_cetak('tunihilsp2d/edit/' . $data->dokid . '/pdf');
		...
		//drupal_goto('tunihilsp2d/edit/' . $dokid . '/pdf');
		if (isSuperuser())
			$rows[] = array(
...
function tunihilsp2d_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["tunihilsp2d_arsip_kodeuk"] = $kodeuk;
	$_SESSION["tunihilsp2d_arsip_bulan"] = $bulan;
	$_SESSION["tunihilsp2d_arsip_sp2dok"] = $sp2dok;

	$uri = 'tunihil_sp2d/filter/' . $kodeuk . '/' . $bulan . '/' . $sp2dok ;
	drupal_goto($uri);
	
}


function tunihilsp2d_arsip_main_form($form, &$form_state) {
