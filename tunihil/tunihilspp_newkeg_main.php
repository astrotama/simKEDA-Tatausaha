<?php
function tunihilspp_newkeg_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($kodesuk=='ZZ')
		...
		$str_suk = '| ' . '<a href="/tunihilspp/newkeg/ZZ">SEMUA</a>' . ' |';
	...
	foreach ($query as $data) {
		...
		if ($kodesuk==$data->kodesuk)
			...
			$str_suk .= apbd_blank_space() . '<a href="/tunihilspp/newkeg/' . $data->kodesuk . '">' . $namasuk_menu . '</a>' . ' |';
	...
	foreach ($results as $data) {
		...
		$editlink = apbd_button_baru_custom('tunihilspp/newspj/' . $data->kodekeg, 'SPP'); 
		$rows[] = array(
	...
	//$output_form = drupal_get_form('tunihilspp_newkeg_main_form');
...
function tunihilspp_newkeg_main_form_submit($form, &$form_state) {
	/*
	...
	$_SESSION["tunihilspp_newkeg_bulan"] = $bulan;
	$_SESSION["tunihilspp_newkeg_spjsudah"] = $spjsudah;
	$_SESSION["tunihilspp_newkeg_jenisgaji"] = $jenisgaji;
...
function tunihilspp_newkeg_main_form($form, &$form_state) {
	/*
	$kodeuk = apbd_getuseruk();
	if(arg(2)!=null){
		...
		//$bulan = date('m');
		isset($_SESSION["tunihilspp_newkeg_bulan"])?$bulan = $_SESSION["tunihilspp_newkeg_bulan"]:$bulan = date('m');;
		isset($_SESSION["tunihilspp_newkeg_spjsudah"])?$spjsudah = $_SESSION["tunihilspp_newkeg_bulan"]:$spjsudah = 'ZZ';
		isset($_SESSION["tunihilspp_newkeg_jenisgaji"])?$jenisgaji = $_SESSION["tunihilspp_newkeg_jenisgaji"]:$jenisgaji = 'ZZ';
