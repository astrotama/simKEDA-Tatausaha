...
function tunihilspp_delete_form() {
    ...
    if (isset($dokid)) {
        ...
        if ($ada) {
			...
			if ($current_url != $referer)
				$_SESSION["tunihilspp_lastpage"] = $referer;
			else
				$referer = $_SESSION["tunihilspp_lastpage"];
...
function tunihilspp_delete_form_validate($form, &$form_state) {
...
function tunihilspp_delete_form_submit($form, &$form_state) {
