<?php
function sp2dgajiarsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["sp2dgajiarsip_kodeuk"]))
				$_SESSION["sp2dgajiarsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["sp2dgajiarsip_kodeuk"];
			//if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["sp2dgajiarsip_bulan"]))
				$_SESSION["sp2dgajiarsip_bulan"]='1';
		$bulan = $_SESSION["sp2dgajiarsip_bulan"];
		//if ($bulan=='') $bulan = '1';
		
		if(!isset($_SESSION["sp2dgajiarsip_sp2dok"]))
				$_SESSION["sp2dgajiarsip_sp2dok"]='ZZ';
		$sp2dok = $_SESSION["sp2dgajiarsip_sp2dok"];
		//if ($sp2dok=='') $sp2dok = 'ZZ';
		
		if(!isset($_SESSION["sp2dgajiarsip_jenisgaji"]))
				$_SESSION["sp2dgajiarsip_jenisgaji"]='ZZ';
		$jenisgaji = $_SESSION["sp2dgajiarsip_jenisgaji"];
	...
	$output_form = drupal_get_form('sp2dgajiarsip_main_form');
	if (isSuperuser())
		$header = array (
	...
	$query->fields('d', array('dokid', 'bulan', 'keperluan', 'kodeuk', 'spmno', 'sppno', 'sp2dno', 'sp2dtgl', 'jumlah', 'potongan', 'netto', 'sp2dok', 'jenisgaji'));
...
function sp2dgajiarsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["sp2dgajiarsip_kodeuk"] = $kodeuk;
	$_SESSION["sp2dgajiarsip_bulan"] = $bulan;
	$_SESSION["sp2dgajiarsip_sp2dok"] = $sp2dok;
	$_SESSION["sp2dgajiarsip_jenisgaji"] = $jenisgaji;

	$uri = 'sp2dgajiarsip/filter/' . $kodeuk . '/' . $bulan . '/' . $sp2dok . '/' . $jenisgaji;
	drupal_goto($uri);
	
}


function sp2dgajiarsip_main_form($form, &$form_state) {
