    ...
    if (isset($dokid)) {
        ...
        if ($ada) {
			...
			if ($current_url != $referer)
				$_SESSION["sppnihil_lastpage"] = $referer;
			else
				$referer = $_SESSION["sppnihil_lastpage"];
...
function sppnihil_delete_form_validate($form, &$form_state) {
	$dokid = $form_state['values']['dokid'];
	
	$results = db_query('select count(kodero) as rekening from dokumenrekening where dokid=:dokid', array(':dokid' => $dokid));
	foreach($results as $data){
		$rekening = $data->rekening;
	}
	if ($rekening >= 0){
		form_set_error($rekening,'Penghapusan Gagal Karena Masih Ada Kegiatan');
	}
}
function sppnihil_delete_form_submit($form, &$form_state) {
