<?php
function sppnihilarsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		if (isUserSKPD()) 
			...
			if(!isset($_SESSION["sppnihilarsip_kodeuk"]))
				$_SESSION["sppnihilarsip_kodeuk"]='ZZ';
			$kodeuk = $_SESSION["sppnihilarsip_kodeuk"];
			//if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		if(!isset($_SESSION["sppnihilarsip_bulan"]))
				$_SESSION["sppnihilarsip_bulan"]= date('m');
		$bulan = $_SESSION["sppnihilarsip_bulan"];
		if ($bulan=='') $bulan = date('m');
		
		
		if(!isset($_SESSION["sppnihilarsip_sppok"]))
				$_SESSION["sppnihilarsip_sppok"]='ZZ';
		$sppok = $_SESSION["sppnihilarsip_sppok"];
		//if ($sppok=='') $sppok = 'ZZ';

		$sppnihil = $_SESSION["sppnihilarsip_sppnihil"];
		if ($sppnihil=='') $sppnihil = '0';
	...
	*/
	...
	$output_form = drupal_get_form('sppnihilarsip_main_form');
	if (isSuperuser()) 
		$header = array (
			...
			array('data' => 'Jumlah', 'width' => '90px', 'field'=> 'netto',  'valign'=>'top'),
	...
	$query->fields('d', array('dokid', 'sppno', 'spptgl','jenisdokumen', 'spmno', 'sp2dno', 'bulan', 'kodeuk', 'jumlah', 'potongan', 'netto', 'sppok', 'keperluan', 'sp2dok', 'sp2dsudah'));
	$query->fields('u', array('namasingkat'));
	
	//GAJI
	if (($sppnihil=='5') || ($sppnihil=='7')) {
		$query->condition('d.jenisdokumen', $sppnihil, '=');
	} else {
	...
	foreach ($results as $data) {
		...
		if ($data->jenisdokumen =='5') {
			$editlink = apbd_button_jurnal('gubaruspp/edit/' . $data->dokid);
		} else {
			$editlink = apbd_button_jurnal('tunihilbaruspp/edit/' . $data->dokid);
		}
		//$editlink = apbd_button_jurnal('tunihilbaruspp/edit/' . $data->dokid);
		if($data->sppok=='0') $editlink .=  apbd_button_hapus('sppnihil/delete/' . $data->dokid);		//'<a class="btn //btn-danger btn-xs 
		//	$editlink .=  apbd_button_hapus_disabled();
		//else
		//	$editlink .=  apbd_button_hapus('sppnihil/delete/' . $data->dokid);		//'<a class="btn btn-danger btn-xs 
	...
	/*
	if (isUserSKPD()) {
		$btn = apbd_button_baru('tunihilspp/newkeg');
...
function sppnihilarsip_main_form_submit($form, &$form_state) {
	$kodeuk = $form_state['values']['kodeuk'];
	$bulan = $form_state['values']['bulan'];
	$sppok = $form_state['values']['sppok'];
	$sppnihil = $form_state['values']['sppnihil'];
	...
	*/
	...
	$_SESSION["sppnihilarsip_kodeuk"] = $kodeuk;
	$_SESSION["sppnihilarsip_bulan"] = $bulan;
	$_SESSION["sppnihilarsip_sppok"] = $sppok;
	$_SESSION["sppnihilarsip_sppnihil"] = $sppnihil;
	
	$uri = 'sppnihilarsip/filter/' . $kodeuk . '/' . $bulan  . '/' . $sppok . '/' . $sppnihil ;
	drupal_goto($uri);
	
}


function sppnihilarsip_main_form($form, &$form_state) {
	...
	$form['formdata'] = array (
		'#type' => 'fieldset',
		'#title'=>  'PILIHAN DATA' . '<em><small class="text-info pull-right">' . get_label_data($bulan, $sppok) . '</small></em>',		//'#attributes' => array('class' => array('container-inline')),
		'#collapsible' => TRUE,
		'#collapsed' => FALSE,        
	);		
	...
	//SPP
	$opt_nihil['0'] ='SEMUA SPP NIHIL';
	$opt_nihil['7'] = 'SPP TU NIHIL';	
	$opt_nihil['5'] = 'SPP GU NIHIL';
	$form['formdata']['sppnihil'] = array(
		'#type' => 'select',
		'#title' =>  t('Jenis SPP Nihil'),
		'#prefix' => '<div class="col-md-6">',
		'#suffix' => '</div>',
		'#options' => $opt_nihil,
		//'#default_value' => isset($form_state['values']['skpd']) ? $form_state['values']['skpd'] : $kodeuk,
		'#default_value' => $sppnihil,
	);	 
