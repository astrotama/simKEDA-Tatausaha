<?php
function gunihilspp_newspj_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){		
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('gunihilspp_newspj_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function gunihilspp_newspj_main_form($form, &$form_state) {
...
function gunihilspp_newspj_main_form_validate($form, &$form_state) {
	//$sppno = $form_state['values']['sppno'];
		
}
	
function gunihilspp_newspj_main_form_submit($form, &$form_state) {
    ...
    drupal_goto('gunihilspp/new/' . $kodekeg);
