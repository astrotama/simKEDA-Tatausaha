<?php
function pfkspp_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		//$bulan = date('m');
		$bulan = $_SESSION["pfkspp_arsip_bulan"];
		if ($bulan=='') $bulan = date('m');
		
		$sppok = $_SESSION["pfkspp_arsip_sppok"];
		if ($sppok=='') $sppok = 'ZZ';
	...
	*/
	...
	$output_form = drupal_get_form('pfkspp_arsip_main_form');
	$header = array (
	...
	foreach ($results as $data) {
		...
		$editlink = apbd_button_jurnal('pfkspp/edit/' . $data->dokid);
		if($data->sppok=='0') $editlink .=  apbd_button_hapus('pfkspp/delete/' . $data->dokid);
...
function pfkspp_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["pfkspp_arsip_kodeuk"] = $kodeuk;
	$_SESSION["pfkspp_arsip_bulan"] = $bulan;
	$_SESSION["pfkspp_arsip_sppok"] = $sppok;
	
	$uri = 'pfkspparsip/filter/' . $kodeuk . '/' . $bulan  . '/' . $sppok;
	drupal_goto($uri);
	
}


function pfkspp_arsip_main_form($form, &$form_state) {
