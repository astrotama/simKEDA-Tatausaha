<?php
function pfksp2d_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		//$bulan = date('m');
		$bulan = $_SESSION["pfksp2d_arsip_bulan"];
		if ($bulan=='') $bulan = '1';
		
		$sp2dok = $_SESSION["pfksp2d_arsip_sp2dok"];
		if ($sp2dok=='') $sp2dok = 'ZZ';
	}
	...
	$output_form = drupal_get_form('pfksp2d_arsip_main_form');
	if (isSuperuser())
		$header = array (
	...
	*/
	...
	foreach ($results as $data) {
		...
		$esp2dlink = "eSP2D";
		$editlink = apbd_button_jurnal('pfksp2d/edit/' . $data->dokid);
		...
		if($data->sp2dok=='2')
			...
			//CETAK
			//if ((isSuperuser()) and  ($data->sp2dno!='')) $editlink .= apbd_button_cetak('pfksp2d/edit/' . $data->dokid . '/pdf');
		...
		//drupal_goto('pfksp2d/edit/' . $dokid . '/pdf');
		if (isSuperuser()) {
			
			//CETAK
			if (($data->sp2dok=='1') and  ($data->sp2dno!='')) 
				$cetaklink = apbd_button_cetak('pfksp2d/edit/' . $data->dokid . '/pdf');
			else
				$cetaklink = 'Cetak';
	...
	$btn .= '<em class="text-info pull-right">Nomor Terakhir : ' . apbd_getmaxnosp2d('8', '0') . '</em>';
...
function pfksp2d_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["pfksp2d_arsip_kodeuk"] = $kodeuk;
	$_SESSION["pfksp2d_arsip_bulan"] = $bulan;
	$_SESSION["pfksp2d_arsip_sp2dok"] = $sp2dok;

	$uri = 'pfksp2darsip/filter/' . $kodeuk . '/' . $bulan . '/' . $sp2dok ;
	drupal_goto($uri);
	
}


function pfksp2d_arsip_main_form($form, &$form_state) {
	...
	*/
	...
	if(arg(2)!=null){
		...
		if (isUserSKPD()) 
			...
			$kodeuk = $_SESSION["pfksp2d_arsip_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		$bulan = $_SESSION["pfksp2d_arsip_bulan"];
