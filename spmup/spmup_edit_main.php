<?php
function spmup_edit_main($arg=NULL, $nama=NULL) {
	...
	if (arg(3)=='pdf-e') {			
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('spmup_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function spmup_edit_main_form($form, &$form_state) {

	//FORM NAVIGATION	
	$current_url = url(current_path(), array('absolute' => TRUE));
	$referer = $_SERVER['HTTP_REFERER'];
	if ($current_url != $referer)
		$_SESSION["spmuplastpage"] = $referer;
	else
		$referer = $_SESSION["spmuplastpage"];
	...
	$query->fields('d', array('dokid', 'sppno', 'spmno', 'spmtgl','spptgl', 'kodekeg', 'bulan', 'keperluan', 'jumlah', 'potongan', 'netto', 
			'penerimanama', 'penerimabankrekening', 'penerimabanknama', 'penerimanpwp', 'pptknama', 'pptknip', 'sp2dok', 'spmok', 'cetakspm', 'approved'));
	...
	// drupal_set_message("tanggal spp :$spptgl - spm : $spmtgl");
	// drupal_set_message("tanggal spp :$spptglx - spm : $spmtglx");
	...
	} elseif (($spmok=='1') and ($sp2dok=='0') and ($approved=='0')) {	
...
function spmup_edit_main_form_validate($form, &$form_state) {
...
if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
	...
	drupal_goto('spmup/delete/' . $dokid);
	...
	//	//watchdog_exception('spmup-' . $nourut, $e);
	//}
	
	if($form_state['clicked_button']['#value'] == $form_state['values']['submitspmok']) 
		drupal_goto('spmup/verify/' . $dokid);
...
function printspm($dokid, &$kodeuk) {
	...
	$jenisbelanjastr = '';
	if ($kodeuk=='00') $jenisbelanjastr = 'TIDAK ';
	$rows[]=array(
		array('data' => 'Jenis Belanja', 'width' => '150','align'=>'left','style'=>'border-left:1px solid black;'),
		array('data' => ':', 'width' => '10px','align'=>'center','style'=>'border:none'),
		array('data' => 'BELANJA ' . $jenisbelanjastr . 'LANGSUNG', 'width' => '350px','align'=>'left','style'=>'border-right:1px solid black;'),
	);
	...
	$rows[]=array(
		...
		array('data' => 'Uang Persediaan', 'width' => '270px','align'=>'left','style'=>'border-right:1px solid black;'),
