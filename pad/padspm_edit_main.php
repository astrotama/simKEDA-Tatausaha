<?php
function padspm_edit_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){		
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('padspm_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function padspm_edit_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'arsip')>0)
		$_SESSION["padspmlastpage"] = $referer;
	else
	...
	foreach ($results as $data) {
		...
		$title = 'SPM Restitusi - '  . $data->keperluan ;
...
function padspm_edit_main_form_validate($form, &$form_state) {
...
if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
	...
	/*
		...
		watchdog_exception('padspm-' . $nourut, $e);
	}
	*/
	
	if($form_state['clicked_button']['#value'] == $form_state['values']['submitspmok']) 
		drupal_goto('padspm/verify/' . $dokid);

	//drupal_goto('padspmarsip');
	//drupal_goto();
}	
}
...
function printspm($dokid) {
	...
	$rows[]=array(
		...
		array('data' => ':', 'width' => '10px','align'=>'center','style'=>'border:none'),
		array('data' => $penerimanama, 'width' => '350px','align'=>'left','style'=>'border-right:1px solid black;'),
	);
	$rows[]=array(
	...
	$rows[]=array(
		...
		array('data' => ':', 'width' => '10px','align'=>'center','style'=>'border:none'),
		array('data' => 'PENGELUARAN RESTITUSI PAD', 'width' => '350px','align'=>'left','style'=>'border-right:1px solid black;'),
	);
