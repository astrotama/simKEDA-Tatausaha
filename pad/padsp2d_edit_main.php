<?php
function padsp2d_edit_main($arg=NULL, $nama=NULL) {
	...
	if (arg(3)=='pdf0') {			
		...
		//$output .= theme('pager');
		//drupal_set_message();
		$output_form = drupal_get_form('padsp2d_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function padsp2d_edit_main_form($form, &$form_state) {

	//FORM NAVIGATION	
	$current_url = url(current_path(), array('absolute' => TRUE));
	$referer = $_SERVER['HTTP_REFERER'];
	if ($current_url != $referer)
		$_SESSION["padsp2dlastpage"] = $referer;
	else
		$referer = $_SESSION["padsp2dlastpage"];
	...
	$query->fields('d', array('dokid', 'spmno', 'sp2dno', 'sp2dtgl', 'kodekeg','kodebank', 'bulan', 'keperluan', 'jumlah', 'potongan', 'netto', 
	...
	if (isSuperuser()) {
		if ($nosudah) {
			...
			$form['SP2D'] = array (
				'#type' => 'fieldset',
				'#title'=> 'SP2D<em class="text-info pull-right">Nomor Terakhir : ' . apbd_getmaxnosp2d('3', '4') . '</em>',
				'#collapsible' => TRUE,
				'#collapsed' => FALSE,        
			);	
		
			if (arg(3)=='auto') {
				$sp2dno = apbd_getnosp2d('6', '0');
			}
...
function padsp2d_edit_main_form_validate($form, &$form_state) {
...
if($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
	...
	$sp2dno = $form_state['values']['sp2dno'];
	//if (apbd_is_duplikasi($sp2dno)) 
	//	form_set_error('sp2dno', 'Nomor SP2D sudah ada. Ganti dengan nomor lain, bisa diketik manual atau klik tombol Otomatis');
	//else {
		...
		try {	
			...
			atchdog_exception('padsp2d-' . $nourut, $e);
		}
	//}
}	
}
