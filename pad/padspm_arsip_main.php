<?php
function padspm_arsip_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 10;
	...
	if ($arg) {
		...
		$_SESSION["padspm_arsip_kodeuk"] = $kodeuk;
		$_SESSION["padspm_arsip_bulan"] = $bulan;
		$_SESSION["padspm_arsip_spmok"] = $spmok;
		
	} else {
		if (isUserSKPD()) 
			$kodeuk = apbd_getuseruk();
		else {
			$kodeuk = $_SESSION["padspm_arsip_kodeuk"];
			if ($kodeuk=='') $kodeuk = 'ZZ';
		}
		//$bulan = date('m');
		$bulan = $_SESSION["padspm_arsip_bulan"];
		if ($bulan=='') $bulan = date('m');
		
		$jenisdokumen = $_SESSION["padspm_arsip_jenisdokumen"];
		if ($jenisdokumen=='') $jenisdokumen = 'ZZ';

		$spmok = $_SESSION["padspm_arsip_spmok"];
		if ($spmok=='') $spmok = 'ZZ';
	}
	...
	$output_form = drupal_get_form('padspm_arsip_main_form');
	if (isSuperuser())
		$header = array (
	...
	*/
	...
	foreach ($results as $data) {
		...
		$editlink = apbd_button_jurnal('padspm/edit/' . $data->dokid);
...
function padspm_arsip_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$uri = 'padspmarsip/filter/' . $kodeuk . '/' . $bulan . '/' . $spmok ;
	drupal_goto($uri);
	
}


function padspm_arsip_main_form($form, &$form_state) {
