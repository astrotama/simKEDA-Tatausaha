    ...
    if (isset($dokid)) {
        ...
        if ($ada) {
			...
			if ($current_url != $referer)
				$_SESSION["padspp_lastpage"] = $referer;
			else
				$referer = $_SESSION["padspp_lastpage"];
...
function padspp_delete_form_validate($form, &$form_state) {
}
function padspp_delete_form_submit($form, &$form_state) {
