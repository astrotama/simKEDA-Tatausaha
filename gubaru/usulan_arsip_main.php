...
function usulan_arsip_main($arg=NULL, $nama=NULL) {
	...
	$query = db_select('spjgu', 's')->extend('PagerDefault')->extend('TableSort');
	...
	foreach ($results as $data) {
		...
		if ($data->posting=='1') {
			...
			$editlink1=  apbd_button_hapus('gubaru/delete/' . $data->spjid);
		}
	...
	$btn = '';
	$btn = apbd_button_baru('gubaruspp/pilihkeg/usulan') . "&nbsp;";
...
function usulan_arsip_main_form_submit($form, &$form_state){
	...
	$url = 'gubaru/arsip/filter/' . $skpd . '/' . $bulan . '/' . $posting;
	drupal_goto($url);
}
