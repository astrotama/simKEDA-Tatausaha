...
function gubarusppcoba_edit_main($arg=NULL, $nama=NULL) {
	...
	if($print=='spp1') {			  
		...
		$edoc = 'http://simkedajepara.web.id/edoc2019/spm/E_SPTJB_' . $dokid . '.PDF';
		if (is_eDocExistscoba($edoc)) {
			drupal_goto($edoc); 
...
function gubaruspp_edit_main_form($form, &$form_state) {
	...
	*/
	...
	foreach ($reskeg as $datakeg) {
		$arr_kegiatan[$datakeg->kodekeg] = $datakeg->kegiatan;
	}
	//REKENING
	$form['kegiatan'] = array (
		'#type' => 'select',
		//'#title'=> $datakeg->kegiatan,
		'#options' => $arr_kegiatan,
	...
	$form['wrapperkegiatan']['table']= array(
	...
	if (isset($form_state['values']['kegiatan'])) {
		$kodekeg = $form_state['values']['kegiatan'];
		$dokid = $_SESSION["kegiatan_dokid"];
	} else {
		$kodekeg = '2019C7741012';
		$dokid = $_SESSION["kegiatan_dokid"];
	}
	...
	*/
	//$results = $query->execute();
	$results = db_query('select ro.kodero, ro.uraian, di.jumlah, di.kodekeg, a.anggaran from dokumenrekening as di inner join anggperkeg as a on di.kodero=a.kodero and di.kodekeg=a.kodekeg inner join rincianobyek as ro on di.kodero=ro.kodero where di.dokid=:dokid and di.kodekeg=:kodekeg', array(':dokid'=>$dokid , ':kodekeg' => $kodekeg));
	foreach ($results as $data) {
			$i++; 
			...
			$form['wrapperkegiatan']['table']['nomor' . $i]= array( 
					'#prefix' => '<tr><td>',
					'#markup' => $i,
					//'#size' => 10,
					'#suffix' => '</td>',
			); 
			$form['wrapperkegiatan']['table']['kodero' . $i]= array(
					'#prefix' => '<td>',
					'#markup' => $data->kodero,
					'#size' => 10,
					'#suffix' => '</td>',
			); 
			$form['wrapperkegiatan']['table']['uraian' . $i]= array(
				//'#type'         => 'textfield', 
				'#prefix' => '<td>',
				'#markup'=> $uraian, 
				'#suffix' => '</td>',
			); 
			$form['wrapperkegiatan']['table']['anggaran' . $i]= array(
				//'#type'         => 'textfield', 
				'#prefix' => '<td>',
				'#markup'=> '<p class="text-right">' . apbd_fn($data->anggaran) . '</p>', 
				'#suffix' => '</td>',
			); 
			$form['wrapperkegiatan']['table']['cair' . $i]= array(
				//'#type'         => 'textfield', 
				'#prefix' => '<td>',
				'#markup'=> '<p class="text-right">' . apbd_fn($cair) . '</p>', 
				'#suffix' => '</td>',
			); 
			$form['wrapperkegiatan']['table']['batas' . $i]= array(
				//'#type'         => 'textfield', 
				'#prefix' => '<td>',
				'#markup'=> '<p class="text-right">' . apbd_fn($batas) . '</p>', 
				'#suffix' => '</td>',
			); 		 
			$form['wrapperkegiatan']['table']['jumlahapbdhtml' . $i]= array(
				'#prefix' => '<td>',
				'#markup'=> '<p class="text-right">' . apbd_fn($data->jumlah) . '</p>', 
				'#suffix' => '</td>',
			);			
			
			$linkprint1 = apbd_link_print_small('/kuitansi/edit/' . $dokid . '/' . $datakeg->kodekeg . $data->kodero, '');
			$linkprint2 = apbd_link_print_small('/kuitansi/edita2/' . $dokid . '/' . $data->kodero, '');

			$form['wrapperkegiatan']['table']['printa1' . $i]= array(
				'#prefix' => '<td>',
				'#markup'=> $linkprint1, 
				'#suffix' => '</td>',
			);	
			$form['wrapperkegiatan']['table']['printa2' . $i]= array(
				'#prefix' => '<td>',
				'#markup'=> $linkprint2, 
				'#suffix' => '</td></tr>',
			);	
	...
	//TOTAL KEG
		...
		$form['wrapperkegiatan']['table']['jumlahapbdhtml' . $i]= array(
			'#prefix' => '<td>',
			'#markup'=> '<p class="text-right"><strong>' . apbd_fn($totalkeg) . '</strong></p>', 
			'#suffix' => '</td>',
		);			
		...
		$usulan =  'Usulan'; //l('Usulan', 'gubaru/edit/' . $dokid . '/spp3/' . $datakeg->kodekeg, array('attributes' => array('class' => null)));
		$a21 =  'A2-1';  //l('A2-1', 'gubaru/edit/' . $dokid . '/spp3/' . $datakeg->kodekeg, array('attributes' => array('class' => null)));
		...
		//modal_delete($dokid, $btn, $text, $button, $header=null)
		if ($sppok=='0')
			$delete_link = modal_delete($dokid . $kodekeg, 'Hapus', 'Apakah anda akan menghapus kegiatan <strong>"' . $datakeg->kegiatan .'"</strong> dari usulan SPP?', array('data'=>'Ya','link'=>'gubaruspp/edit/' . $dokid. '/remove/' . $kodekeg));	
		else
	...
	$edocsptjb = 'http://simkedajepara.web.id/edoc2019/spm/E_SPTJB_' . $dokid . '.PDF';
	if (is_eDocExistscoba($edocsptjb)) {
		$form['formdata']['submitsptjb']= array(
			'#type' => 'submit',
