...
function usulan_selectspj_main_form($form, &$form_state) {
	...
	$result = db_query('SELECT s.spjid, s.kodeuk, s.kodekeg, s.tanggal, s.keperluan, s.jumlah, s.posting, d.spptgl, d.sppno, s.tglakhir, s.tglawal, s.spdno, s.bk5url, s.bk6url, s.spp2url FROM spjgu s left join dokumen d ON s.dokid=d.dokid WHERE s.spjid =:spjid', array(':spjid' => $spjid));
...
function usulan_selectspj_main_form_submit($form, &$form_state) {
	...
	$result = db_query('SELECT kodero, jumlah FROM spjgurekening WHERE spjid =:spjid', array(':spjid' => $spjid));
	foreach($result AS $data){
