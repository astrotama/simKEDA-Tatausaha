...
function gukuitansi_a22_main($arg=NULL, $nama=NULL) {
	...
	if($print=='a22') {			
		...
		$output_form = drupal_get_form('gukuitansi_a22_main_form');
		return drupal_render($output_form);// . $output;
	}		
		
}

function gukuitansi_a22_main_form($form, &$form_state) {
	...
	$query = db_select('spjgu', 'b');
	$query->join('spjgurekening', 'bi', 'b.spjid=bi.spjid');
...
function gukuitansi_a22_main_form_submit($form, &$form_state) {
	...
	drupal_goto('gukuitansi/edita2/' . $spjid . '/' . $kodero . '/a22/' . $tanggal . '/'. $penerimanama . '/'. $penerimanalamat . '/' . 
