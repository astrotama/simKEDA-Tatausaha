...
function usulan_delete_form() {
    ...
    if (isset($spjid)) {
        ...
        //if ($ada) {
			...
			$form['formdata'] = array (
				'#type' => 'fieldset',
				'#title'=> 'Konfirmasi Penghapusan GU',
				'#collapsible' => TRUE,
				'#collapsed' => FALSE,        
			);
			...
			if ($current_url != $referer)
				$_SESSION["gubaru_lastpage"] = $referer;
			else
				$referer = $_SESSION["gubaru_lastpage"];

			return confirm_form($form,
								'Anda yakin menghapus GU Nomor : ' . $kodekeg . ' tanggal : ' . $tanggal . ' keperluan : ' . $keperluan ,
								$referer,
