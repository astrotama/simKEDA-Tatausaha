...
function gubaruspm_edit_main_form($form, &$form_state) {
	...
	foreach ($results as $data) {
		...
		$sppno = 'SPP : ' . l($data->sppno . ', ' . apbd_fd_long($data->spptgl), 'gubaruspp/edit/' . $dokid, array('attributes' => array('class' => null)));
	...
	if ($spmok=='1') {
		if (isAdministrator()) {
			$form['formatas']['submitprint-reset']= array(
				'#type' => 'submit',
		...
		if ($cetakspm=='0') {
			...
			$form['formatas']['submitprint-e']= array(
				'#type' => 'submit',
			...
			$form['formatas']['submitprint-s']= array(
				'#type' => 'submit',
				'#value' => '<span class="glyphicon glyphicon-print" aria-hidden="true"></span>eSPM',
				'#attributes' => array('class' => array('btn btn-success btn-sm pull-right')),
			);
	...
	$form['formatas']['submitprint']= array(
		'#type' => 'submit',
	...
	$form['formspmno'] = array(
		'#prefix' => '<div class="col-md-6">',
		'#suffix' => '</div>',				
	);
		$form['formspmno']['sppmo_title'] = array(
			'#markup' => '<small>Nomor SPM</small>',
		);	
		$form['formspmno']['spmno'] = array(
			'#type' => 'textfield',
			//'#title' =>  t('No SPM'),
			// The entire enclosing div created here gets replaced when dropdown_first
			// is changed.
	...
	$form['formspmtgl'] = array(
		'#prefix' => '<div class="col-md-6">',
		'#suffix' => '</div>',				
	);	
		$form['formspmtgl']['spmtgl_title'] = array(
			'#markup' => '<small>Tanggal SPM</small>',
		);
		$form['formspmtgl']['spmtgl']= array(
	...
	foreach ($reskeg as $datakeg) {
		...
		$form['formkegiatan']['wrapperkegiatan'. $nokeg] = array(
			'#prefix' => '<div id="kegiatan'.$nokeg.'-wrapper"  class="col-md-12">',
			'#suffix' => '</div>',
		);
