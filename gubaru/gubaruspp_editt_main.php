...
function gubaruspp_edit_main_form($form, &$form_state) {
	...
	$arr_keg = $reskeg->fetchAllAssoc('kodekeg');
	foreach ($arr_keg as $datakeg) {
		$nokeg++;
		$totalkeg = 0;
		//REKENING
		...
		$arr_ro = $results->fetchAllAssoc('kodero');
		foreach ($arr_ro as $data) {
			$i++; 
		...
		$form['formrekening' . $datakeg->kodekeg]['show_command_satu' . $datakeg->kodekeg] = array(
			'#title' => t("BK 5 "),
			'#type' => 'checkbox',
			'#default_value' => FALSE,
			'#ajax' => array(
		...
		$form['formrekening' . $datakeg->kodekeg]['display' . $datakeg->kodekeg] = array(
			'#type' => 'item',
	...
	$form['formkelengkapan']['tablekelengkapan']= array(
		'#prefix' => '<div class="table-responsive"><table class="table"><tr><th width="10px">NO</th><th>URAIAN</th><th width="5px"></th><th width="50px"></th><th width="50px"></th></tr>',
		 '#suffix' => '</table></div>',
	);	
	$i = 0;
	$query = db_select('dokumenkelengkapan', 'dk');
	$query->join('ltkelengkapandokumen', 'lt', 'dk.kodekelengkapan=lt.kodekelengkapan');
	$query->fields('dk', array('kodekelengkapan', 'url'));
	...
	foreach ($results as $data) {
		...
		if ($data->tipe=='0') {
			$link_upload = l('Upload', 'upload/dok/' . $dokid . '/' . $kode, array('html'=>true));
			if ($data->url=='') {
				$link_buka =  'Tampilkan';
				$simbol = apbd_icon_belumada();
				
			} else {
				$link_buka = l('Tampilkan', $data->url, array('html'=>true));	
		...
		$form['formkelengkapan']['tablekelengkapan']['simbol' . $i]= array(
			//'#type'         => 'textfield', 
			'#prefix' => '<td>',
			'#markup'=> $simbol, 
			'#suffix' => '</td>',
		); 
		$form['formkelengkapan']['tablekelengkapan']['tipe' . $i]= array(
			'#prefix' => '<td>',
			'#markup' => $link_upload,
			'#suffix' => '</td>',
		);	
	...
	*/
	...
	if ($sppok=='0') {
		...
		$form['formdata']['submitcetak']= array(
			'#type' => 'submit',
...
function ajax_mainindex_callback($form, $form_state) {
	...
	drupal_set_message('idnex . ' . $index);
