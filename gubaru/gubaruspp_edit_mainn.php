...
function gubaruspp_edit_main_form($form, &$form_state) {
	...
	*/
	...
	foreach ($reskeg as $datakeg) {
		...
		$res_spj = db_query('select spjid from spjgu where dokid=:dokid and kodekeg=:kodekeg', array(':dokid'=>$dokid, ':kodekeg'=>$datakeg->kodekeg));
		...
		//modal_delete($dokid, $btn, $text, $button, $header=null)
		if ($sppok=='0')
			$delete_link = modal_delete($dokid . $datakeg->kodekeg, 'Hapus', 'Apakah anda akan menghapus kegiatan <strong>"' . $datakeg->kegiatan .'"</strong> dari usulan SPP?', array('data'=>'Ya','link'=>'gubaruspp/edit/' . $dokid. '/remove/' . $datakeg->kodekeg));	
		else
	...
	$edocsptjb = 'E_SPTJB_' . $dokid . '.PDF';
	if (is_eDocExistsBaru($edocsptjb)) {
