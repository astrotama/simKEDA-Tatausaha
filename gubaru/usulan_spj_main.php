<?php
function usulan_spj_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){		
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('usulan_spj_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function usulan_spj_main_form($form, &$form_state) {
...
function usulan_spj_main_form_validate($form, &$form_state) {
	//$sppno = $form_state['values']['sppno'];
		
}
	
function usulan_spj_main_form_submit($form, &$form_state) {
